...
Search queries:"""
    ...
    def filter_results(query: str, results: List[Dict[str, Any]]) -> str:
        ...
        if is_domain_query:
            ...
            domain_warning = f"""
IMPORTANT: The investigation query is about domain/company "{domain_name}" (main part: "{main_domain}").

ACCEPTANCE RULES:
- ACCEPT results that mention "{domain_name}" (exact match)
- ACCEPT results that mention "{main_domain}" in relevant context
- ACCEPT results about related companies/brands if they are the same entity (e.g., T-Mobile and tcell.tj may refer to the same company)
- ACCEPT results with variations of the domain/company name (e.g., "tcell", "t-cell", "T-Mobile", "tmobile")

Focus on finding security-related information, data breaches, vulnerabilities, or threats related to "{domain_name}" or its related brands/entities.
"""
...
- If the query is about a specific domain/company, results should mention that domain/company or related brands/entities
- Accept results about related companies/brands if they refer to the same entity
- Relevance score should be HIGH (0.7+) for results that directly match the query domain/company
- Relevance score should be MEDIUM (0.4-0.7) for results about related brands/entities that may refer to the same company
- Relevance score should be LOW (0.0-0.3) for results that are clearly about different, unrelated companies
